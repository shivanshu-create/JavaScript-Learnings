let Score = JSON.parse(localStorage.getItem('score')) || {
    ...
    function makePopUp(){
      const pop = document.querySelector('.noShow');
      pop.classList.remove('noShow');
      let show = '';
      for(let i = 0;i < 3;i++){
        show += `<div onclick="
          AutoPlay(${i+1});
          document.querySelector('.showThePop').classList.add('noShow');
        ">${i+1} Sec</div>`
      }
      pop.innerHTML = show;
    }
    ...
    document.addEventListener('click', function(event) {
      const pop = document.querySelector('.showThePop');
      const autoplay = document.querySelector('.askTime');
      if(!autoplay.contains(event.target) && !pop.contains(event.target)){
        document.querySelector('.showThePop').classList.add('noShow');
      }
    });